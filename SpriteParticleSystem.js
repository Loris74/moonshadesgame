...
SpriteParticleSystem = function (param)
{
  ...
  {
    //var sprite = new THREE.Sprite( { map: this.texture, useScreenCoordinates: false } );
    
    //var mapA = THREE.ImageUtils.loadTexture( "assets/textures/particles/Particle_ex1.png" );
    
    //var sm = new THREE.SpriteMaterial();
    //var sm = new THREE.MeshPhongMaterial( { map: this.texture} );
    
    //sm.map = this.texture;
    
    var sprite = new THREE.Sprite( {map:this.texture, useScreenCoordinates: false, color: 0xffffff, fog: true} );
    
    //console.log(sprite);
    //sprite.useScreenCoordinates = false;
    //sprite.visible = false;
    //sprite.color = new THREE.Color( 0xff0000 ); 
    
    //sprite.mergeWith3D = true;
...
SpriteParticleSystem.prototype.psUpdate = function()
{
  ...
  for (var i=0;i<this.particles.length;++i)
  {
    ...
    //sprite.uvScale.set( scale, scale );
    
    sprite.opacity = 1.0 - fracLife;
  }
  
  while (now>=this.nextSpawnT)
  {
